...
function flyTo(map, coord, duration, resolution) {
  ...
  map.getView().animate(
    // { duration: 3000, resolution: resolution },
    { duration: 3000, center: coord }
  );
}
...
OpenStreet.prototype.getParameters = function getParameters (opt) {
  return {
    ...
    params: {
      ...
      // polygon_geojson: opt.polygon_geojson || this.settings.params.polygon_geojson,
...
Nominatim.prototype.chosen = function chosen (place, addressHtml, addressObj, addressOriginal) {
  ...
  // console.log(ol.extent.applyTransform(bbox, ol.proj.getTransform("EPSG:4326", "EPSG:3857")));
  // if (bbox) {
  ...
  if (this.options.preventDefault === true) {
    ...
    if (bbox) {
        var minlon = parseFloat(bbox[3]);
        var minlat = parseFloat(bbox[0]);
        var maxlon = parseFloat(bbox[2]);
        var maxlat = parseFloat(bbox[1]);

        var extent = [minlon, minlat, maxlon, maxlat];
        extent = ol.extent.applyTransform(extent, ol.proj.getTransform("EPSG:4326", "EPSG:3857"));
        map.getView().fit(extent, { duration: 2500 });
    } else {
      flyTo(map, coord);
    }
